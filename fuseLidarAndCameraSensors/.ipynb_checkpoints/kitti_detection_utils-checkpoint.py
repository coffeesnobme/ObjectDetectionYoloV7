...
def get_detection_coordinates(image, bin_path, model, T_velo_cam, draw_boxes=True, draw_depth=True):
    ...
    # Compute detections using YOLOv11
    ...
    if draw_boxes:
        ...
        plt.imshow(detections[0].plot())  # YOLOv11 uses .plot() for visualization
        plt.show()

    # get bounding box locations (x1,y1), (x2,y2), Prob, class
    bboxes = detections[0].boxes.xyxy.cpu() # Extract bounding boxes
